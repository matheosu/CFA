...
struct Response WifiClient::sendData(String request, int timeout) {
    ...
    if (find(command(sendCommand, 5000), ">")) {
        ...
        // Status
        int indexLN = response.indexOf("\n");
        String s = response.substring(0, indexLN);
        s.replace("HTTP/1.1","");
        int serverIndex = s.indexOf("Server:");
        if(serverIndex != -1){
            String codeResponse = s.substring(0, serverIndex);
            codeResponse.trim();
            r.status = codeResponse.toInt();
        }

        // Content Type
        response = response.substring(indexLN+1, response.length());
        indexLN = response.indexOf("\n");
        String contentType = response.substring(0, indexLN);
        ...
        response = response.substring(indexLN+1, response.length());
        indexLN = response.indexOf("\n");
        String contentLength = response.substring(0, indexLN);
        contentLength.replace("Content-Length:","");
        contentLength.trim();
        r.length = contentLength.toInt();

        // Message
        response = response.substring(response.indexOf("Connection:"), response.length());
        response.replace("Connection: close\r\n\r\n","");
        r.message = response;
